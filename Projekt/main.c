...
int main(void)
{
  ...
  mpz_lcm(lambda, p_1, q_1);
	...
	char* wiadomosc = (char*) calloc (MAX_SIZE/8, sizeof(char));
	getMessage(wiadomosc);
	char* hex = (char*) calloc (2*strlen(wiadomosc),sizeof(char));
	convertTextToHex(wiadomosc,hex); //konwertujemy wiadomosc do systemu hex
	...
	char* wynik = (char*) calloc (1000,sizeof(char));
	mpz_get_str(wynik, 16, decrypted); 
	char* odkodowane = (char*) calloc(1000, sizeof(char));
